...
(ns cloned.clojure.tools.analyzer.js.cljs.core
  ...
  (:require clojure.walk
            ...
            [cloned.clojure.tools.analyzer :as a]
            [cloned.clojure.tools.analyzer.utils :as utils]))
  ...
  ([x & next]
    (let [forms (concat [x] next)]
      (if (every? #(simple-test-expr? &env %)
             (map #(cloned.clojure.tools.analyzer.js/analyze % &env) forms))
...
(defn resolved-name [{:keys [ns] :as env} sym]
  (let [sym-ns (namespace sym)
        sym-ns (if (= "cloned.clojure.tools.analyzer.js.cljs.core" sym-ns)
                 "cljs.core"
                 sym-ns)]
    (cond
     (= 'Object sym)
  ...
  ([& kvs]
     (let [keys (map first (partition 2 kvs))]
       (if (core/and (every? #(= (:op %) :const)
                        (map #(cloned.clojure.tools.analyzer.js/analyze % &env) keys))
...
(defmacro hash-set
  ...
  ([& xs]
     (if (core/and (core/<= (count xs) 8)
                   (every? #(= (:op %) :const)
                      (map #(cloned.clojure.tools.analyzer.js/analyze % &env) xs))
...
(defmacro ns-interns
  ...
  `(into {} ~(mapv (fn [[sym {:keys [ns name]}]] `[~sym (var ~(core/symbol (core/name ns) (core/name name)))])
