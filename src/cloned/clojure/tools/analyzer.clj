...
(ns cloned.clojure.tools.analyzer
   ...
   * var?
   ...
   Setting up the global env is also required, see cloned.clojure.tools.analyzer.env
  ...
  (:require [cloned.clojure.tools.analyzer.utils :refer :all]
  ...
  (let [op (first form)]
    ...
    (let [mform (macroexpand-1 form env)]
      (if (= form mform) ;; function/special-form invocation
        (parse mform env)
        (-> (analyze-form mform env)
          (update-in [:raw-forms] (fnil conj ()) {:zform (first form) :zvar (ns-resolve *ns* env op)})
          #_(update-in [:raw-forms] (fnil conj ())
